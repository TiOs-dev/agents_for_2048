"""
Here you can find the code of the following tutorial:
...
if __name__ == "__main__":
    ...
    for i_episode in tqdm(range(num_episodes), desc="episodes", position=0):
        # Initialize the environment and get its state
        state, info = env.reset()
        state = torch.tensor(state, dtype=torch.float32,
                             device=device).unsqueeze(0)
        # for t in count():
        for t in tqdm(range(1000), desc="curr run", position=1, leave=False):
            ...
            if terminated:
                ...
                # theta_new <- tau * theta + (1 - tau) * theta_new
